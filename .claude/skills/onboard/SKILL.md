---
name: onboard
description: Use on the FIRST session of this AI OS — automatically, before responding to any other request — and whenever someone says "set me up", "onboard me", "install my OS", or opens this folder for the first time. The full installer: FIRST does the strict setup in a fixed order with no skipping — install the plugins (Superpowers, claude-mem, context-mode), connect Obsidian (Local REST API MCP), and connect Google Workspace via Claude's built-in browser connector — THEN learns about the user via optional questions OR a file they drop in to be ingested, and scaffolds the Day-1 files. Setup is strict; the intake is optional. Resumable across restarts and idempotent.
---
...
1. **Phase 0 — Setup first (strict).** Install the plugins (Superpowers, claude-mem, context-mode; + skill-creator), **connect Obsidian** (Local REST API MCP), and **connect Google** (Gmail / Calendar / Drive) via Claude's built-in connector — **before anything else**, in a fixed order, no skipping, no asking which to do first. On the first session this runs automatically, before any other request.
...
**You do the heavy lifting, not the user.** The only things the user installs by hand are Claude Code and Node.js (see `INSTALL-FIRST.md`). Inside this skill: plugin installs are slash commands the user types; Google is a browser click; and for Obsidian, **you run the `claude mcp add` command for them** once they paste their key — they don't type terminal commands.

**Resumable.** Plugins and the Obsidian MCP need a Claude Code restart, so this skill is built to be re-run. After a restart the user just runs `/onboard` again (or says "continue onboarding") — each run re-checks state and continues from the first unmet step.
...
**Goal of this phase:** plugins installed (**Superpowers, claude-mem, context-mode**; + skill-creator) · **Obsidian connected** (Local REST API MCP) · **Google connected** (Gmail / Calendar / Drive). Work them in this fixed order, automatically — **do not ask which to set up first, do not offer to skip to the questions.** Do not enter Phase 1 until setup passes.
...
- **Node.js present?** `command -v node` (Node also provides `npx`, which the Obsidian MCP needs). If missing, **offer to install it for them** by running this repo's installer — `bash setup.sh` (macOS/Linux) or `powershell -ExecutionPolicy Bypass -File setup.ps1` (Windows). It installs Node LTS via nvm / winget (no Homebrew). Then have them reopen Claude Code and re-run `/onboard`. (Manual alternative: `INSTALL-FIRST.md` step 2.)
...
- **Obsidian:** check `~/.claude.json` for an `"obsidian"` entry under `mcpServers` (global or this project). A quick grep is enough — don't dump the whole file.
...
```
Done. Your AI OS is set up (plugins + Obsidian + Google) and now knows who you are, what you sell, what matters this quarter, and how you sound.
...
2. **Resumable across restarts.** Every run re-checks `installed_plugins.json`, the `obsidian` MCP entry in `~/.claude.json`, and the available Google connector tools, and continues from the first unmet step. Re-running never re-does completed work.
3. **You do the terminal work, not the user.** You can't run `/plugin` (user-typed + needs restart) or click the Google browser consent — guide and verify those. You CAN and SHOULD run `claude mcp add obsidian ...` yourself once the user pastes their key. The only by-hand installs are Claude Code + Node.js (`INSTALL-FIRST.md`).
4. **Never echo API keys or OAuth secrets** back in plain text — not in summaries, not in recaps.
5. **Setup is strict and runs first, in fixed order — no choosing, no skipping.** Plugins (Superpowers, claude-mem, context-mode) + Obsidian + Google must pass before the interview. The *only* exception is a genuinely-blocked Google, recorded as `blocked:` in `connections.md`.
...
- Cold-test: fresh clone, type "hi" on the first session. Expected: the OS does NOT answer "hi" — it starts Phase 0, walks plugin install → restart → resume, runs `claude mcp add obsidian` for the user after they paste the key, connects Google in the browser, and refuses the interview until plugins + Obsidian + Google pass.
- Resume test: install plugins, restart, re-run `/onboard`. Expected: Plugins ✓ from `installed_plugins.json`, Obsidian ✓ from the `obsidian` MCP entry; proceeds without re-asking.
- Key-safety test: paste an Obsidian key. Expected: it runs `claude mcp add` and never prints the key back.
